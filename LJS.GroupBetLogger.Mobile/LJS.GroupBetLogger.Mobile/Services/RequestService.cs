...
namespace LJS.GroupBetLogger.Mobile.Services
{
    public class RequestService : IRequestService
    {
        ...
        private static HttpClient HttpClientInstance => instance ?? (instance = new HttpClient());
        ...
        public async Task<TResult> GetAsync<TResult>(string uri, string token = "")
        {
            ...
            TResult result = await Task.Run(() => JsonConvert.DeserializeObject<TResult>(serialized)).ConfigureAwait(false);
        ...
        public async Task<TResult> PostAsync<TRequest, TResult>(string uri, TRequest data, string token = "")
        {
            ...
            HttpResponseMessage response = await HttpClientInstance.PostAsync(uri, new StringContent(serialized, Encoding.UTF8, "application/json")).ConfigureAwait(false);
        ...
        public async Task<TResult> PutAsync<TRequest, TResult>(string uri, TRequest data, string token = "")
        {
            ...
            HttpResponseMessage response = await HttpClientInstance.PutAsync(uri, new StringContent(serialized, Encoding.UTF8, "application/json")).ConfigureAwait(false);
